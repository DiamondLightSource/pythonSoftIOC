...
def test_value_retrieval_pre_init_set(
    ...
    assert out_rec.get() == expected_value
    assert type(out_rec.get()) == expected_type


def run_test_function(expected_value, expected_type, inner_func):
    """Helper function to handle the multiprocessing process"""
    queue = multiprocessing.Queue()
    process = multiprocessing.Process(target=inner_func, args=(queue,))
    process.start()

    try:
        rec_val = queue.get(timeout=5)

        assert rec_val == expected_value
        assert type(rec_val) == expected_type
    finally:
        process.join()
...
def test_value_retrieval_post_init_set(record_values):
    ...
    run_test_function(expected_value, expected_type, inner_func)
